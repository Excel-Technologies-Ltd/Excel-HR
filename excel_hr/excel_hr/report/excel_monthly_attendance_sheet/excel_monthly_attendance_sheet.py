...
def execute(filters: Optional[Filters] = None) -> Tuple:
    ...
    if not attendance_map:
        frappe.msgprint(_("No attendance records found."),
    ...
    if filters.summarized_view:
        columns.extend(
            [
                ...
                {"label": _("Total Leaves"), "fieldname": "total_leaves",
                 "fieldtype": "Float", "width": 110},
                {"label": _("Total Absent"), "fieldname": "total_absent",
                 "fieldtype": "Float", "width": 110},
                # {
        ...
        columns.append({"label": _("Shift"), "fieldname": "shift",
                       "fieldtype": "Data", "width": 120})
...
def get_columns_for_leave_types() -> List[Dict]:
    ...
    for entry in leave_types:
        types.append(
            {"label": entry, "fieldname": frappe.scrub(
                entry), "fieldtype": "Float", "width": 120}
        )
...
#             Employee.excel_department
#         )
#         .where(Employee.company == filters.company)
#     )
#     print(filters)
#     # if filters.employee:
#     #     regex_pattern = '|'.join(map(re.escape, filters.employee))
#     #     query = query.append(["employee", "REGEXP", regex_pattern])
#     if filters.employee:
#         query += " WHERE employee IN ('ETL14090047')"
#     if filters.excel_department:
#         query = query.where(Employee.excel_department == filters.excel_department)
#     if filters.excel_section:
#         query = query.where(Employee.excel_section == filters.excel_section)
#     if filters.excel_sub_section:
#         query = query.where(Employee.excel_sub_section == filters.excel_sub_section)
#     if filters.excel_job_location:
#         query = query.where(Employee.excel_job_location == filters.excel_job_location)
#     # this is new above
...
def get_attendance_status_for_summarized_view(
    ...
    """
    summary, attendance_days = get_attendance_summary_and_days(
        employee, filters)
...
def get_attendance_summary_and_days(employee: str, filters: Filters) -> Tuple[Dict, List]:
    ...
    absent_case = frappe.qb.terms.Case().when(
        Attendance.status == "Absent", 1).else_(0)
    sum_absent = Sum(absent_case).as_("total_absent")

    leave_case = frappe.qb.terms.Case().when(
        Attendance.status == "On Leave", 1).else_(0)
...
def get_leave_summary(employee: str, filters: Filters) -> Dict[str, float]:
    ...
    """
    Attendance = frappe.qb.DocType("Attendance")
    day_case = frappe.qb.terms.Case().when(
        Attendance.status == "Half Day", 0.5).else_(1)
